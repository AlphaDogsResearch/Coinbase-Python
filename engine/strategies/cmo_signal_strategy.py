...
from engine.database.models import build_cmo_signal_context
...
class CMOSignalStrategyConfig:
    """CMO Signal Strategy configuration."""
...
class CMOSignalStrategy(Strategy):
    """
    CMO Signal Strategy Implementation.

    Uses the Chande Momentum Oscillator (CMO) for signal generation.
    CMO ranges from -100 to +100. Signals fire when CMO crosses the
    upper/lower band thresholds.
    """

    VALID_SIGNAL_MODES = {"mean_reversion", "momentum"}
    VALID_EXIT_MODES = {"midpoint", "breakout"}

    def __init__(self, config: CMOSignalStrategyConfig) -> None:
        super().__init__(config)

        self.config = config

        # CMO Parameters
        self.cmo_period = config.cmo_period
    ...
    def on_candle_created(self, candle: MidPriceCandle):
        ...
        (
            ...
            long_exit_signal,
            short_exit_signal,
        ) = self._compute_signals(current_cmo)

        if self.cache.is_flat(self.instrument_id):
            if self._cooldown_left == 0:
                if long_entry_signal:
                    self._enter_long(candle, current_cmo, reason="CMO long entry")
        ...
        self._previous_cmo = current_cmo
        self._bars_processed += 1

    def _compute_signals(self, current_cmo: float):
        mr_long_signal = (
            self._previous_cmo < self.cmo_lower and current_cmo >= self.cmo_lower
        )
        mr_short_signal = (
            self._previous_cmo > self.cmo_upper and current_cmo <= self.cmo_upper
        )
        ...
        mr_long_mid_exit = (
            self._previous_cmo < self.cmo_mid and current_cmo >= self.cmo_mid
        )
        mr_short_mid_exit = (
            self._previous_cmo > self.cmo_mid and current_cmo <= self.cmo_mid
        )
    ...
    def _handle_long_position(
        ...
        if long_exit_signal:
            if self._close_position(candle, "CMO midpoint exit"):
                self._stopped_out_count = 0
            return
    ...
    def _build_signal_context(
        ...
        return build_cmo_signal_context(
            reason=reason,
            cmo=current_cmo,
            prev_cmo=self._previous_cmo,
            cmo_upper=self.cmo_upper,
            cmo_lower=self.cmo_lower,
            cmo_mid=self.cmo_mid,
