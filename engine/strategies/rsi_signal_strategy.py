...
from engine.database.models import build_rsi_signal_context
...
class RSISignalStrategyConfig:
    """RSI Signal Strategy configuration."""
...
class RSISignalStrategy(Strategy):
    """
    RSI Signal Strategy Implementation
    """

    VALID_SIGNAL_MODES = {"mean_reversion", "momentum"}
    VALID_EXIT_MODES = {"midpoint", "breakout"}

    def __init__(self, config: RSISignalStrategyConfig) -> None:
        ...
        if self.cache.is_flat(self.instrument_id):
            if self._cooldown_left == 0:
                if long_entry_signal:
                    self._enter_long(candle, current_rsi, reason="RSI long entry")
    ...
    def _compute_signals(self, current_rsi: float):
        mr_long_signal = (
            self._previous_rsi < self.rsi_lower and current_rsi >= self.rsi_lower
        )
        mr_short_signal = (
            self._previous_rsi > self.rsi_upper and current_rsi <= self.rsi_upper
        )
        ...
        mr_long_mid_exit = (
            self._previous_rsi < self.rsi_mid and current_rsi >= self.rsi_mid
        )
        mr_short_mid_exit = (
            self._previous_rsi > self.rsi_mid and current_rsi <= self.rsi_mid
        )
    ...
    def _handle_long_position(
        ...
        if long_exit_signal:
            if self._close_position(candle, "RSI midpoint exit"):
                self._stopped_out_count = 0
            return
