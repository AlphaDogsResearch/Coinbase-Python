...
from engine.database.models import build_trix_signal_context
...
class TRIXSignalStrategyConfig:
    """TRIX Signal Strategy configuration."""
    ...
    # TRIX Parameters
    trix_period: int = 6
    trix_upper: float = 0.0676
    trix_lower: float = -0.0522
    trix_mid: float = -0.0193
...
class TRIXSignalStrategy(Strategy):
    """
    TRIX Signal Strategy Implementation.

    Uses the TRIX indicator (1-period percentage rate of change of triple EMA)
    for signal generation. Signals fire when TRIX crosses the upper/lower
    band thresholds.
    """
    ...
    def __init__(self, config: TRIXSignalStrategyConfig) -> None:
        super().__init__(config)

        self.config = config

        # TRIX Parameters
        self.trix_period = config.trix_period
        self.trix_upper = config.trix_upper
        self.trix_lower = config.trix_lower
        self.trix_mid = config.trix_mid
        ...
        self.trix = TRIX(period=self.trix_period)

        # State tracking
        self._previous_trix = 0.0
    ...
    def on_candle_created(self, candle: MidPriceCandle):
        ...
        self.trix.handle_bar(candle)
        if not self.trix.initialized:
            return

        current_trix = self.trix.value
        ...
        (
            ...
            long_exit_signal,
            short_exit_signal,
        ) = self._compute_signals(current_trix)

        if self.cache.is_flat(self.instrument_id):
            if self._cooldown_left == 0:
                if long_entry_signal:
                    self._enter_long(candle, current_trix, reason="TRIX long entry")
        ...
        self._previous_trix = current_trix
    ...
    def _handle_long_position(
        ...
        if long_exit_signal:
            if self._close_position(candle, "TRIX midpoint exit"):
                self._stopped_out_count = 0
            return
    ...
    def _build_signal_context(
        ...
        return build_trix_signal_context(
            reason=reason,
            trix=current_trix,
            prev_trix=self._previous_trix,
            trix_upper=self.trix_upper,
            trix_lower=self.trix_lower,
            trix_mid=self.trix_mid,
