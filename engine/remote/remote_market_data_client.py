...
class RemoteMarketDataClient(MarketDataClient):
    def __init__(self):
        self.port = 5555  # Must match gateway's market_data_connection_port
        self.name = "Remote Market Data Connection"
        self.order_book_listeners: Dict[str, List[Callable[[OrderBook], None]]] = (
            {}
        )  # list of callbacks
    ...
    def notify_order_book_listeners(self, order_book: OrderBook):
        ...
        listeners = self.order_book_listeners.get(symbol, [])
        if not listeners:
            logging.warning(
                f"{self.name} [Market Data] No listeners registered for symbol {symbol}. "
                f"Available symbols: {list(self.order_book_listeners.keys())}"
            )
            return
        logging.debug(
            f"{self.name} [Market Data] Notifying {len(listeners)} listener(s) for {symbol}"
        )
        for listener in listeners:
            try:
                ...
                logging.error(
                    f"{self.name} [Market Data] Listener raised an exception: {e}", exc_info=True
                )
    ...
    def notify_tick_listeners(self, order_book: OrderBook):
        ...
        for listener in self.tick_price_listener:
            try:
                ...
                logging.error(self.name + "[Market Data] Listener raised an exception: %s", e)
