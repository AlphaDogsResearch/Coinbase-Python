...
class PairConnection:
    ...
    def log_current_stats(self):
        ...
        self.logger.info(f"[{self.name}] === Connection Statistics ===")
        ...
        self.logger.info(f"  Messages Dropped: {self.stats.messages_dropped} ({drop_rate:.2f}%)")
        self.logger.info(f"  ZMQ Again Errors: {self.stats.zmq_again_errors} ({zmq_again_rate:.2f}%)")
        self.logger.info(f"  Average Send Latency: {self.stats.avg_send_latency * 1000:.2f}ms")
        self.logger.info(f"  Time since last send: {time_since_last_send:.2f}s")
        self.logger.info(f"  Time since last receive: {time_since_last_receive:.2f}s")

        if self.stats.message_types_sent:
            self.logger.info("  Message Types Sent:")
            for msg_type, count in sorted(self.stats.message_types_sent.items(), key=lambda x: x[1], reverse=True):
                ...
                self.logger.info(f"    {msg_type}: {count} ({percentage:.1f}%)")
        ...
        if self.drop_history:
            ...
            self.logger.info("  Recent Drop Events:")
            for drop in recent_drops:
                time_str = time.strftime("%H:%M:%S", time.localtime(drop['time']))
                self.logger.info(f"    {time_str} - {drop['type']} (ZMQ.Again: {drop['zmq_again']})")

        self.logger.info(f"[{self.name}] === End Statistics ===\n")
    ...
    def get_socket_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Error getting socket status: {e}")
            return {}
    ...
    def start_receiving(self, callback):
            ...
            while self.running:
                try:
                    if self.type == zmq.ROUTER:
                        ...
                        self.logger.info(f"[{self.name}] Received from {identity}: {msg}")
                        callback(msg)
                    else:
                        ...
                        try:
                            ...
                            if hasattr(e, 'message'):
                                self.logger.error("Receiver loop message %s",e.message,exc_info=True)
                            else:
                                self.logger.error("Receiver loop %s",e,exc_info=True)
        ...
        self.logger.info(f"[{self.name}] Started receiver and stats monitor threads")
    ...
    def _send_with_monitoring(self, obj: Any, obj_type: str = "Unknown"):
        ...
        try:
            ...
            self.logger.error(f"[{self.name}] ZMQ.Again - Dropped {obj_type}: {obj}")
            return False
        except Exception as e:
            latency = time.time() - start_time
            self._update_send_stats(obj_type, success=False, latency=latency)
            self.logger.error(f"[{self.name}] Error sending {obj_type}: {e}")
            return False
    ...
    def send(self, message: str):
        ...
        if not success:
            self.logger.error(f"Dropped string message: {message}")

    def send_wallet_response(self, wallet_response: WalletResponse):
        self.logger.info(f"[{self.name}] Sending Wallet Response: {wallet_response}")
        success = self._send_with_monitoring(wallet_response, "WalletResponse")
        if not success:
            self.logger.error(f"Dropped WalletResponse: {wallet_response}")

    def send_wallet_request(self, wallet_request: WalletRequest):
        self.logger.info(f"[{self.name}] Sending Wallet Request: {wallet_request}")
        success = self._send_with_monitoring(wallet_request, "WalletRequest")
        if not success:
            self.logger.error(f"Dropped WalletRequest: {wallet_request}")

    def send_account_response(self, account_response: AccountResponse):
        self.logger.info(f"[{self.name}] Sending Account Response: {account_response}")
        success = self._send_with_monitoring(account_response, "AccountResponse")
        if not success:
            self.logger.error(f"Dropped AccountResponse: {account_response}")

    def send_account_request(self, account_request: AccountRequest):
        self.logger.info(f"[{self.name}] Sending Account Request: {account_request}")
        success = self._send_with_monitoring(account_request, "AccountRequest")
        if not success:
            self.logger.error(f"Dropped AccountRequest: {account_request}")

    def send_position_response(self, position_response: PositionResponse):
        self.logger.info(f"[{self.name}] Sending Position Response: {position_response}")
        success = self._send_with_monitoring(position_response, "PositionResponse")
        if not success:
            self.logger.error(f"Dropped PositionResponse: {position_response}")

    def send_position_request(self, position_request: PositionRequest):
        self.logger.info(f"[{self.name}] Sending Position Request: {position_request}")
        success = self._send_with_monitoring(position_request, "PositionRequest")
        if not success:
            self.logger.error(f"Dropped PositionRequest: {position_request}")

    def send_margin_info_response(self, margin_info_response: MarginInfoResponse):
        self.logger.info(f"[{self.name}] Sending Margin Info Response: {margin_info_response}")
        success = self._send_with_monitoring(margin_info_response, "MarginInfoResponse")
        if not success:
            self.logger.error(f"Dropped MarginInfoResponse: {margin_info_response}")

    def send_margin_info_request(self, margin_info_request: MarginInfoRequest):
        self.logger.info(f"[{self.name}] Sending Margin Info Request: {margin_info_request}")
        success = self._send_with_monitoring(margin_info_request, "MarginInfoRequest")
        if not success:
            self.logger.error(f"Dropped MarginInfoRequest: {margin_info_request}")

    def send_commission_rate_response(self, commission_rate_response: CommissionRateResponse):
        self.logger.info(f"[{self.name}] Sending Commission Rate Response: {commission_rate_response}")
        success = self._send_with_monitoring(commission_rate_response, "CommissionRateResponse")
        if not success:
            self.logger.error(f"Dropped CommissionRateResponse: {commission_rate_response}")

    def send_commission_rate_request(self, commission_rate_request: CommissionRateRequest):
        self.logger.info(f"[{self.name}] Sending Commission Rate Request: {commission_rate_request}")
        success = self._send_with_monitoring(commission_rate_request, "CommissionRateRequest")
        if not success:
            self.logger.error(f"Dropped CommissionRateRequest: {commission_rate_request}")

    def send_trades_response(self, trades_response: TradesResponse):
        self.logger.info(f"[{self.name}] Sending Trades Response: {trades_response}")
        success = self._send_with_monitoring(trades_response, "TradesResponse")
        if not success:
            self.logger.error(f"Dropped TradesResponse: {trades_response}")

    def send_trades_request(self, trades_request: TradesRequest):
        self.logger.info(f"[{self.name}] Sending Trades Request: {trades_request}")
        success = self._send_with_monitoring(trades_request, "TradesRequest")
        if not success:
            self.logger.error(f"Dropped TradesRequest: {trades_request}")


    def send_reference_data_response(self, reference_data_response: ReferenceDataResponse):
        self.logger.info(f"[{self.name}] Sending Reference Data Response: {reference_data_response}")
        success = self._send_with_monitoring(reference_data_response, "ReferenceDataResponse")
        if not success:
            self.logger.error(f"Dropped ReferenceDataResponse: {reference_data_response}")

    def send_reference_data_request(self, reference_data_request: ReferenceDataRequest):
        self.logger.info(f"[{self.name}] Sending Reference Data Request: {reference_data_request}")
        success = self._send_with_monitoring(reference_data_request, "ReferenceDataRequest")
        if not success:
            self.logger.error(f"Dropped Reference Data Request: {reference_data_request}")

    def send_trade(self, trade: Trade):
        self.logger.info(f"[{self.name}] Sending Trade: {trade}")
        success = self._send_with_monitoring(trade, "Trade")
        if not success:
            self.logger.error(f"Dropped Trade: {trade}")

    def send_order(self, order: Order):
        """Send an order to the peer."""
        self.logger.info(f"[{self.name}] Sending Order: {order}")
        success = self._send_with_monitoring(order, "Order")
        if not success:
            self.logger.error(f"Dropped Order: {order}")

    def publish_order_event(self, order_event: OrderEvent):
        self.logger.info(f"[{self.name}] Sending Order: {order_event}")
        success = self._send_with_monitoring(order_event, "OrderEvent")
        if not success:
            self.logger.error(f"Dropped OrderEvent: {order_event}")

    def publish_market_data_event(self, order_book: OrderBook):
        """Send a market data to the peer."""
        self.logger.debug(f"[{self.name}] Sending Market Data: {order_book}")
        success = self._send_with_monitoring(order_book, "OrderBook")
        if not success:
            self.logger.error(f"Dropped OrderBook: {order_book}")

    def publish_mark_price(self, mark_price: MarkPrice):
        """Send a mark price to the peer."""
        self.logger.debug(f"[{self.name}] Sending Mark Price: {mark_price}")
        success = self._send_with_monitoring(mark_price, "MarkPrice")
        if not success:
            self.logger.error(f"Dropped MarkPrice: {mark_price}")

    def enable_monitoring(self, enabled: bool = True):
        """Enable or disable monitoring"""
        self.monitoring_enabled = enabled
        if enabled:
            self.logger.info(f"[{self.name}] Monitoring enabled")
        else:
            self.logger.info(f"[{self.name}] Monitoring disabled")
    ...
    def set_stats_interval(self, interval_seconds: int):
        ...
        self.logger.info(f"[{self.name}] Stats logging interval set to {interval_seconds} seconds")
    ...
    def stop(self):
        ...
        self.socket.close()
        self.context.term()
        self.logger.info(f"[{self.name}] Connection stopped")