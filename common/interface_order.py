...
# An executing order
@dataclass
class Order(Serializable, JsonModel, Recyclable, ABC):
    order_id: str
    side: Side
    leaves_qty: float
    symbol: str
    ...
    avg_filled_price: float = field(default=0)
    total_amount: float = field(default=0)
    is_in_order_done_state: bool = field(default=False)
    is_id_init: bool = field(default=False)
    ...
    def initialize(self, order_id:str):
        self.order_id = order_id
        self.is_id_init = True
    ...
    def on_filled_event(self,event_filled_quantity:float,event_filled_price:float) -> None:
        ...
        if self.leaves_qty == 0:
            self.state_changed_to(OrderStatus.FILLED)
    ...
    def on_order_cancel_event(self):
        ...
        self.is_in_order_done_state = True
        self.comment = "Order Cancelled"

    def state_changed_to(self, new_state:OrderStatus):
...
if __name__ == "__main__":
    order = Order.create_base_order("A1234")
    print(order.to_json_external_with_time_stamp())
...
class OrderEvent(Serializable):
    ...
    def __str__(self):
        return "Order event [contract={} ,client_order_id={}, order_id={}, status={}, type={}, side={}, last_filled_price={}, last_filled_qty={}, canceled_reason={}]"\
            .format(self.contract_name,self.client_order_id, self.order_id, self.status, self.execution_type, self.side, self.last_filled_price, self.last_filled_quantity, self.canceled_reason)
...
class AccountEvent(Serializable):
    """
    Futures user stream ACCOUNT_UPDATE payload (USDS-M / COIN-M).
    Balances and positions retain Binance field names in nested dicts.
    """
    ...
    def __init__(
        ...
        self.futures_unit = futures_unit
        self.account_alias = account_alias

    @classmethod
    def from_binance_account_update(cls, data: dict) -> "AccountEvent":
        a = data.get("a") or {}
        return cls(
            ...
            futures_unit=data.get("fs"),
            account_alias=data.get("i") or None,
        )

    def __str__(self):
        return (
            f"AccountEvent[E={self.event_time}, T={self.transaction_time}, reason={self.reason!r}, "
            f"fs={self.futures_unit!r}, balances={len(self.balances)}, positions={len(self.positions)}]"
        )
...
class Trade(Serializable):
    ...
    def __str__(self):
        return "Trade=" + str(self.order_id) + \
            ", Contract Name=" + self.contract_name + \
            ", Side=" + str(self.side) + \
            ", Price=" + str(self.price) + \
            ", Size=" + str(self.size) + \
            ", Received Time=" + str(self.received_time) + \
            ", Realized Pnl=" + str(self.realized_pnl) + \
            ", Liquidation" + str(self.liquidation)


class OrderSizeMode(Enum):
    QUANTITY = "quantity"   # size in base currency
    NOTIONAL = "notional"   # size in quote currency